...
for(const id of ['sedan','suv','van','retro']){
  console.log(id, await S(`window.__GAME.forceSeg(0,'${id}')`));
  await new Promise(r=>setTimeout(r,1200));
  await S("(()=>{const g=window.__GAME;const p=g.carPos(0);g.pose(p.x+5.0, 0.75, p.z+3.4, 0, 0); g.lookAt(p.x, 0.55, p.z, 6.0);})()");
  await new Promise(r=>setTimeout(r,2200));
  await pg.screenshot({path:`progress/artifacts/game_${id}.png`});
}
