...
await pg.goto('http://127.0.0.1:8176/',{waitUntil:'load',timeout:120000});
...
await pg.evaluate("document.getElementById('hud').style.visibility='hidden'; document.getElementById('goals').style.visibility='hidden';");
await new Promise(r=>setTimeout(r,5000));
// force one arrival in bay 0 so the foreground has a car
await pg.evaluate("window.__GAME.clearBay(0); window.__GAME.forceArr(0);");
await new Promise(r=>setTimeout(r,9000));
await pg.evaluate("window.__GAME.lookAt(-6.6,0.9,-3.1, 7)");
await pg.evaluate("window.__GAME.weather(1)");
await new Promise(r=>setTimeout(r,3500));
await pg.screenshot({path:'/home/lex/chargebay/progress/artifacts/scenery_rain.png'});
console.log('shot ERRORS:', errs.length?[...new Set(errs)].slice(0,4).join(' | '):'none');
