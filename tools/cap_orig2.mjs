...
// side view from port side (-X): compute from live car pos via carPos hook
await S("(()=>{const g=window.__GAME;const p=g.carPos(0);g.pose(p.x-5.8,0.9,p.z,0,0);g.lookAt(p.x,0.5,p.z,5.8);})()");
await new Promise(r=>setTimeout(r,2400));
await pg.screenshot({path:'progress/artifacts/orig_side.png'});
// front 3/4 from port-front octant
await S("(()=>{const g=window.__GAME;const p=g.carPos(0);g.pose(p.x-4.4,1.0,p.z+4.4,0,0);g.lookAt(p.x,0.5,p.z,5.6);})()");
await new Promise(r=>setTimeout(r,2200));
await pg.screenshot({path:'progress/artifacts/orig_front.png'});
