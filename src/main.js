...
function animate(){
  ...
  for(const b of sim.bays){
    ...
    if(b.state==='charging' && b.car){
      ...
      if(b.car.userData.battery>=1){ b.state='departing'; b.plugged=false; b.plug.position.copy(b.plugHome.pos); b.plug.rotation.copy(b.plugHome.rot); setJacket(b,false); SFX.cash(); ECONOMY.paySession(b); }
    ...
    if((b.state==='parked'||b.state==='ready') && b.car && !b.car.userData.docked){
      ...
      if(waited>b.car.userData.patience){ b.state='departing'; sim.kickedHourly[(Math.floor(sim.gameClock/60))%24]=(sim.kickedHourly[(Math.floor(sim.gameClock/60))%24]||0)+1; SFX.chime(300,220); SFX.departHorn(); ECONOMY.repAdd(-(b.car.userData.vip?5:2.5)); sim.dayStats.kicks++; ECONOMY.paintGoals(); toast('😠 '+(b.car.userData.seg?b.car.userData.seg.name:'Car')+' left without charging · rep −'+(b.car.userData.vip?5:2.5)); }
    }
  }
